...
WORKDIR /src
COPY ["ApexVolley.csproj", "."]
RUN dotnet restore "./ApexVolley.csproj"
COPY . .
WORKDIR "/src/."
RUN dotnet build "./ApexVolley.csproj" -c $BUILD_CONFIGURATION -o /app/build

# Questa fase viene usata per pubblicare il progetto di servizio da copiare nella fase finale
FROM build AS publish
ARG BUILD_CONFIGURATION=Release
RUN dotnet publish "./ApexVolley.csproj" -c $BUILD_CONFIGURATION -o /app/publish /p:UseAppHost=false
